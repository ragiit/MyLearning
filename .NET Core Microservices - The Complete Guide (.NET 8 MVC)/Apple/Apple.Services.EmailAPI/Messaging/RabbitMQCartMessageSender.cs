...
{
    public class RabbitMqCartConsumer(IConfiguration configuration, IServiceScopeFactory scopeFactory) : IHostedService
    {
        ...
        private async void OnMessageReceived(object? sender, BasicDeliverEventArgs e)
        {
            ...
            var userDto = JsonConvert.DeserializeObject<CartDto>(message);
            ...
            try
            {
                ...
                if (userDto != null)
                {
                    await emailService.EmailCartAndLog(userDto);
                }
            }
            catch (Exception ex)
            {
